...
export class MovieModalComponent implements OnInit{
  ...
  {
    this.loaded = false;
    this.movieInDB.releaseDate = new Date(this.releaseDateNgb.year, this.releaseDateNgb.month - 1, this.releaseDateNgb.day + 1);
  ...
  async updateMovie()
  {
    try 
    {
      ...
      if (result) this.activeModal.close(result)
    }
    catch (e)
    {
