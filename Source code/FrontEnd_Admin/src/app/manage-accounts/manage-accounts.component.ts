...
export class ManageAccountsComponent implements OnInit, OnDestroy {
  ...
  async getAccounts()
  {
    ...
    this.accounts = this.accounts.filter(a => a.username != this.auth.currentAccountValue.username || a.roleName != this.auth.currentAccountValue.roleName);
    if (!this.isSuperAdmin) this.accounts = this.accounts.filter(a => a.roleName != RolesService.superAdmin && a.roleName != RolesService.admin);
  }
  async getRoles()
  {
    let url = this.apiService.backendHost + "/api/Roles";
    this.roles =  await this.http.get<Role[]>(url).toPromise();
    if (this.isSuperAdmin) this.roles = this.roles.filter(r => r.name != RolesService.superAdmin)
    else this.roles = this.roles.filter(r => r.name != RolesService.superAdmin && r.name != RolesService.admin)
    this.lastId = this.roles[this.roles.length - 1].id;
  }
  rerender() 
  {
  ...
  {
    this.roleName = value > this.lastId? 'Tất cả': text;
    if (value > this.lastId) this.rSelectAll = 'Loại người dùng'; 
  ...
  openAddAdminModal()
  {
    ...
    modalRef.componentInstance.modalName = 'Thêm admin'
    modalRef.componentInstance.addWriter = false
  ...
  {
    const modalRef = this.modalService.open(AddAdminModalComponent, {backdrop: 'static', keyboard: false});
    modalRef.componentInstance.modalName = 'Thêm writer'
    modalRef.componentInstance.addWriter = true
    ...
    modalRef.result.then(async (result: any) => 
    {
      if (typeof(result) == 'object') 
      {
        this.accounts.push(result);
        this.filter();
        this.toastSuccess('Thêm tài khoản Writer thành công!');
      }
      else if (result == 'Failed') this.toastError('Thêm tài khoản không thành công!');
    }, (reason: any) => {})
  }
